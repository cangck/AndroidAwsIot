...
public abstract class ClientModule {
    ...
    static OkHttpClient provideClient(Application application, @Nullable OkhttpConfiguration configuration, OkHttpClient.Builder builder, Interceptor intercept
            , @Nullable List<Interceptor> interceptors, @Nullable GlobalHttpHandler handler, ExecutorService executorService, SSLSocketFactory sslSocketFactory) {
        builder
...
//        添加ssl认证
        String[] hosts = {"ec2-54-173-30-46.compute-1.amazonaws.com"};
    ...
    @Provides
    @Singleton
    static SSLSocketFactory provideSSLSocketFactory(Application application) {
        if (application == null) {
        ...
        try {
            ...
            InputStream certificate = application.getApplicationContext().getAssets().open("cucoservice.cer");
            keyStore.setCertificateEntry("cuco", certificateFactory.generateCertificate(certificate));
